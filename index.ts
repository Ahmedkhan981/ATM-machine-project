...
async function accountMenu(currentAccount: Account): Promise<void> {
    const { option } = await inquirer.prompt({
        type: 'list',
        name: 'option',
        message: 'Select an option:',
...
async function fastCash(currentAccount: Account): Promise<void> {
...
async function depositCash(currentAccount: Account): Promise<void> {
    const { amount } = await inquirer.prompt({
        type: 'number',
        name: 'amount',
...
function checkBalance(currentAccount: Account): void {
    console.log(chalk.blue(`Your balance is: dollar. ${currentAccount.balance}$`));
    accountMenu(currentAccount);
}
async function withdrawCash(currentAccount: Account): Promise<void> {
    const { amount } = await inquirer.prompt({
        type: 'number',
        name: 'amount',
